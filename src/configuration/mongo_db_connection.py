...
print("Exists .env:", Path('.env').exists())

load_dotenv()  # Load env variables from .env file
print("MONGODB_URL =", os.getenv("MONGODB_URL"))

uri = os.getenv("MONGODB_URL")
print("repr(MONGODB_URL) =", repr(uri))
if uri is None:
    print("ERROR: Env var is not loaded.")
elif not uri.startswith("mongodb://") and not uri.startswith("mongodb+srv://"):
    print("ERROR: URI does not start with 'mongodb://' or 'mongodb+srv://'")
    print("First 20 chars:", repr(uri[:20]))
    print("Length of URI:", len(uri))
